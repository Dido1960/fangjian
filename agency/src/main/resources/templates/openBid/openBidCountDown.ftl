...
<div class="app">
    ...
    <section>
        <div class="site-demo-button" id="layerDemo" style="margin-bottom: 0;display: none">
            <div class="tlt-btn layui-btn layui-btn-normal" data-method="offset" data-type="r">已签到投标人</div>
        </div>
        <div class="name-num">
...
<form id="join-form" action="${ctx}/staff/joinBidOpen" method="post">
...
<script>
     ...
     */
    function judgmentViewSignInBidder(successCallback, errorCallBack) {
        $.ajax({
            url: "${ctx}/staff/validViewSignInBidder",
            type: "POST",
            data: {
                advanceTime: 30
            },
    ...
    var loadBidder = false;
    setInterval(function () {
        judgmentViewSignInBidder(function (data) {
            if (data) {
                $("#layerDemo").show();
                ajaxShowBidder();
            }
            ...
            success: function (data) {
                if (data.code === "1") {
                    ...
                    for (var i = 0; i < bidders.length; i++) {
                        ...
                        var html = "<li>";
                        html += "<div class='hum-name' style='width: 8%'>" + (i+1) + "</div>";
                        ...
                        html += "<div class='hum-how'>" + (isNull(bidder.bidderOpenInfo.signinTime) ? '<span class="blove-do">未签到</span>' : '<span class="gray-do">已签到</span>') + "</div>";
                        html += "</li>"
