...
<div class="big">
    ...
    <form id="bidder-info" class="index-play layui-form" action="javascript:void(0)">
        ...
        <#if bidSection.bidClassifyCode == 'A08' || bidSection.bidClassifyCode == 'A12'>
            <div class="inp bidder"
                 data-title="bidderMaxPrice">
                <label><#if bidSection.bidClassifyCode =='A08'>招标控制价<#else>最高投标限价</#if></label>
                <input type="text" class="bidder-val layui-bg-gray" readonly placeholder="请输入"
        ...
        <div class="long bidder" data-title="openPlace">开标地点
            <input type="text" placeholder="请输入" class="bidder-val bidOpenPlace" lay-verify="required"
                   lay-reqText="请填写开标地点" value="${tenderDoc.bidOpenPlace}" onblur=" validData()">
        </div>
...
<div>
    <div class="shang">
        <div class="left">
            <div class="left-btn" id="auto_voice" onclick="autoVoice()">自动唱标</div>
            <div class="left-text blue-color totalTime">质询总用时:0分0秒</div>
        </div>
        <div class="center">
            当前共有投标人： <span class="bidder_count">0</span>家
        </div>
        <div class="right">
            <#if lineStatus.decryptionStatus?? && lineStatus.decryptionStatus == 2>
                <#if !lineStatus.questionStatus??>
                    <span class="start layui-btn-danger" onclick="switchQuestionStatus(1)">开启质询</span>
                </#if>
                <#if lineStatus.questionStatus == 1>
                    <span class="start layui-btn-warm" onclick="switchQuestionStatus(2)">结束质询</span>
                </#if>
                <#if lineStatus.questionStatus == 2>
                    <span class="start layui-btn-danger" onclick="switchQuestionStatus(1)">重启质询</span>
                </#if>
            </#if>
        </div>
    </div>
...
<script id="biddingList" type="text/html">
    ...
    <li id="bidder_{{index}}" class="bidder_{{bidder.id}}">
        <div class="bidder" data-title="bidderName" data-id="{{bidder.id}}" data-index="{{index}}">
            ...
            <p class="bidder-val" onclick="thisPlayVoice('{{index}}')" title="{{bidder.bidderName}}">{{bidder.bidderName}}</p>
        </div>
        <div class="bidder" data-title="bidPrice">
            <input type="hidden" class="bidder-val" value="{{bidder.bidderOpenInfo.bidPriceType}}">
        ...
        <div class="bidder" data-title="questionStatus">
                <span class="bidder-val qsChinese">
                    {{# if(dissentStatus == "1"){ }}
                        无异议
...
<script>
    ...
    $(function () {
        //质询结束计算总用时（秒）
        var totalTime = '${lineStatus.questionTime}';
        if (isNull(totalTime)) {
            totalTime = 0;
        }
        ...
        if ('${lineStatus.questionStatus}' == "1") {
            //最后一次解密开始时间
            var lastQuestionStartTime = '${lastQuestionStartTime}';
            //系统当前时间
            var nowTime = new Date('${nowTime}'.replace(/-/g, "/"));
            var startTime = new Date(lastQuestionStartTime.replace(/-/g, "/"));
            ...
            questionTimeInterval = setInterval(function () {
                calQuestionTime(nowTotalTime);
                nowTotalTime ++;
            }, 1000);

        } else {
            calQuestionTime(totalTime);
        }
    });
     ...
     * 计算总的质询时间
     */
    function calQuestionTime(time) {
        var minutes = Math.floor(time / 60);
        var seconds = time % 60;
        $(".totalTime").text("质询总用时:" + minutes + '分' + seconds + '秒');
    }
    ...
    if ('${lineStatus.decryptionStatus}' == '2') {
        // 解密完成后，显示投标人
        showProject();
    }
    ...
    function showProject() {
        ...
        $.ajax({
            url: '${ctx}/staff/listBiddersForSing',
            type: 'post',
            cache: false,
            async: true,
            data: {
     ...
     * 文件唱标
     */
    function fileCursorStatus(data, msg) {
        updateLineStatus(data, msg, '${ctx}/staff/fileCursorPage', $("#fileCursorLi"));
    }
    ...
    function switchQuestionStatus(qs) {
        ...
        window.top.updateLineStatus({
            ...
            "questionStatus": qs,
            "msg": msg
        }, msg, '${ctx}/staff/fileCursorPage', window.top.$("#fileCursorLi"), overQuestion);
