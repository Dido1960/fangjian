...
<#if tenderDoc.controlPrice?? && tenderDoc.controlPrice != ''>
    <div class="down" id="show-control-price">
        <p class="show-money">最高投标限价为：${((tenderDoc.controlPrice)?number)?string(",###.00")} 元</p>
        <p class="show-chines-money"></p>
        <form action="javascript:void(0)" class="layui-form" style="margin-top: 50px;">
            <button id="btn-update" type="button" class="layui-btn btn" onclick="update_price()">修改最高投标限价</button>
            <div id="update-price" style="display: none">
                <input class="control-price" type="number" maxlength="15" autocomplete="off"
                       placeholder="请填写修改招标最高投标限价，（单位：元）" lay-reqText="请填写修改最高投标限价" lay-verify="required|number|plength">
...
<#--新增最高投标限价-->
    <div class="down" id="add-control-price">
        <form action="javascript:void(0)" class="layui-form" style="margin: 0 auto;">
            <input class="control-price" type="number" maxlength="15" autocomplete="off" placeholder="请填写最高投标限价，（单位：元）"
                   lay-reqText="请填写最高投标限价" lay-verify="required|number|plength">
...
<script>
    ...
    function updateTenderDoc() {
        ...
        if (price >= maxControlPrice) {
            layer.msg("最高投标限价超出最大金额限制", {icon: 2});
            return;
        }
        doLoading();
        $.ajax({
    ...
    $(".control-price").on("keyup", function (e) {
        ...
        if (reg1.test(controlPrice)) {
            ...
            $(".show-money").text("招标最高投标限价为：" + controlPrice + " 元");
        } else {
    ...
    function update_price() {
        var _index = window.top.layer.confirm('确定要修改最高投标限价吗？', {
            icon: 3,
            btn: ['确定', '取消'] //按钮
        }, function () {
            // 显示输入框，修改按钮
