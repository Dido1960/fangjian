...
<script>
    ...
    function checkFinishBidOpen(id, userId) {
        parent.layer.open({
            ...
            content: "${ctx}/staff/checkFinishBidOpenPage?id=" + id,
            area: ['800px', '550px'],
            btnAlign: 'c',
            shade: 0.3,
            btn: ['立即结束', '取消'],
    ...
    function finishBidOpen(id, userId) {
        ...
        $.ajax({
            ...
            success: function (data) {
                loadComplete();
                if (data) {
                    parent.layer.confirm('开标已结束，请设置复会时间！', {
                        icon: 3,
                        shade: 0.01,
                        title: '复会',
                        btn: ['前往']
     ...
     * @param id 标段id
     */
    function setResumeTime(id) {
        parent.layer.open({
            type: 2,
            title: ['复会时间设置', 'text-align:center;font-weight:900;font-size:16px;color:rgba(34,49,101,1);'],
            content: '${ctx}/staff/setResumeTime?id=' + id,
            area: ['40%', '50%'],
            btnAlign: 'c',
            shade: 0.01,
            btn: ['立即结束', '取消'],
            btn1: function (index, layero) {
                var body = parent.layer.getChildFrame('body', index);
                var iframeWin = parent.window[layero.find('iframe')[0]['name']];
                iframeWin.updateResume(function () {
                    parent.window.location.reload();
