...
public class SM4 {
    ...
    private void putLong(long n, byte[] b, int i) {
    ...
    private void SM4SetKey(long[] SK, byte[] key) {
        long[] MK = new long[4];
        long[] k = new long[36];
        MK[0] = this.getLong(key, 0);
        MK[1] = this.getLong(key, 4);
        MK[2] = this.getLong(key, 8);
        MK[3] = this.getLong(key, 12);
        k[0] = MK[0] ^ (long) FK[0];
        k[1] = MK[1] ^ (long) FK[1];
        k[2] = MK[2] ^ (long) FK[2];
        k[3] = MK[3] ^ (long) FK[3];
        for (int i = 0; i < 32; i++) {
            k[(i + 4)] = (k[i] ^ this.SM4CalcRK(k[(i + 1)] ^ k[(i + 2)] ^ k[(i + 3)] ^ (long) CK[i]));
            SK[i] = k[(i + 4)];
        }
    }
    ...
    private void SM4OneRound(long[] sk, byte[] input, byte[] output) {
        ...
        ulbuf[0] = this.getLong(input, 0);
        ulbuf[1] = this.getLong(input, 4);
        ulbuf[2] = this.getLong(input, 8);
        ulbuf[3] = this.getLong(input, 12);
        while (i < 32) {
            ulbuf[(i + 4)] = this.SM4F(ulbuf[i], ulbuf[(i + 1)], ulbuf[(i + 2)], ulbuf[(i + 3)], sk[i]);
            i++;
        }
        this.putLong(ulbuf[35], output, 0);
        this.putLong(ulbuf[34], output, 4);
        this.putLong(ulbuf[33], output, 8);
        this.putLong(ulbuf[32], output, 12);
    }
    ...
    public void SM4SetEncKey(SM4Context ctx, byte[] key) throws Exception {
        if (ctx == null) {
    ...
    public void SM4SetDecKey(SM4Context ctx, byte[] key) throws Exception {
        if (ctx == null) {
            throw new Exception("ctx is null!");
        }
    ...
    public byte[] SM4CryptECB(SM4Context ctx, byte[] input) throws Exception {
        ...
        if ((ctx.padding) && (ctx.mode == SM4_ENCRYPT)) {
        ...
        if (ctx.padding && ctx.mode == SM4_DECRYPT) {
