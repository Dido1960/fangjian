...
var is_haveLoad_theme_def = false;
    ...
    var a = layui.jquery, t = layui.laytpl, i = layui.element, n = layui.setter, l = layui.view, s = layui.device(),
        ...
        x = "layui-icon-spread-left", C = "layadmin-side-shrink", k = "LAY-system-side-menu", P = {
            v: "2020 std", req: l.req, exit: l.exit, escape: function (e) {
                ...
                if (!is_haveLoad_theme_def) {
                    /*  $.ajax({
                          url:'/userStyle/setUserStyle',
                          type:'post',
                          cache:false,
                          async: false,
                          success:function(data){
                              console.log("index------"+data.theme);
                              console.log(n);
                              console.log(layui.setter);
                              e[1]=data.theme;
                              e.color=n.theme.color[data.theme]
                    ...
                    var csrf_token;
                    var csrf_header;
                    /*  alert("length"+eles.length);*/
                    for (var i = 0; i < eles.length; i++) {
                        if (eles[i].getAttribute("name") == "_csrf") {
                            csrf_token = eles[i].content;
                        }
                        if (eles[i].getAttribute("name") == "_csrf_header") {
                            csrf_header = eles[i].content;
                        }
                    }
                    if (csrf_token && csrf_header) {
                        xhr.open("post", "/userStyle/setUserStyle", false);
                        ...
                        xhr.send();
                        is_haveLoad_theme_def = true;
                    }

                }

                var i = (n.theme, layui.data(n.tableName)),
                    l = "LAY_layadmin_theme", s = document.createElement("style"),
            ...
            flexible: function (e) {
                ...
                F.note.index = P.popup({
                    ...
                    success: function (e, a) {
                        ...
                        i.val(l).focus().on("blur", function () {
                            var noteValue = this.value;
                            layui.data(n.tableName, {key: "note", value: noteValue})
                            $.ajax({
                                url: '/userStyle/addUserNote',
                                type: 'post',
                                cache: false,
                                async: true,
                                data: {note: noteValue},
