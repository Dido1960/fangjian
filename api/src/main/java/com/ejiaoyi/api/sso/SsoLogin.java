...
public class SsoLogin {
     ...
     * @return
     */
    public JSONObject getUserInfoByCode(String unifiedSocialCreditCode) {
        String token = getToken();

        // 验证账号密码
        map.put("token", token);
        map.put("code", unifiedSocialCreditCode);
        HttpResponseDTO dto = HttpClientUtil.postRaw(ssoParameter.getSsoUrl() + "/api/sso/getCompanyInfoByCode", JSONObject.toJSONString(map));
        JsonData jsonData = JSONObject.parseObject(dto.getContent(), JsonData.class);
