...
<div class="right-cen vers">
   ...
   <#-- <p>3、GEF文件为非必传文件，具体上传格式以招标文件要求为准，请各投标人在保证文件内容安全的情况下，自行斟酌使用。</p>-->
    <div class="kong"></div>
</div>
<div class="right-down" style="display: none">
    ...
    <form class="layui-form" id="file-form">
        ...
        <div class="gefDiv">
            ...
            <@UploadOneTag name="bidderFileInfo.gefId" allowFileSize="1024M"  readonly = "readonly" allowType="*.gef;"
            autocomplete="off" placeholder="请选择gef格式投标文件上传" class="fileInput layui-input" isLocal="true" style="" disabled="disabled">
            </@UploadOneTag>
        ...
        <div class="sgefDiv">
            ...
            <@UploadOneTag name="bidderFileInfo.sgefId" allowFileSize="1024M" readonly = "readonly" allowType="*.sgef;"
            autocomplete="off" placeholder="请选择sgef格式投标文件上传" class="fileInput layui-input" isLocal="true" style="" disabled="disabled">
            </@UploadOneTag>
            <div class="btn" onclick="upLoadThis(this)">上传文件</div>
        </div>
    </form>
...
<script type="text/javascript">
    ...
    function isUpload() {
        ...
        if ('${bidderFileInfo.receiptId}' != '') {
            if ('${gefFile.id}' != null) {
                $gefDiv.find(".fileInput").val('${gefFile.name}');
                $gefDiv.find("input[name = 'bidderFileInfo.gefId']").val('${gefFile.id}');
            }
            if ('${sgefFile.id}' != null) {
                $sgefDiv.find(".fileInput").val('${sgefFile.name}');
                $sgefDiv.find("input[name = 'bidderFileInfo.sgefId']").val('${sgefFile.id}');
            }
            if ('${czrFile.id}' != null) {
    ...
    function formNotNull() {
        ...
        var $gefDiv = $(".gefDiv");
        var $czrDiv = $(".czrDiv");
        var sgef = $sgefDiv.find("input[name = 'bidderFileInfo.sgefId']").val();
        var gef = $gefDiv.find("input[name = 'bidderFileInfo.gefId']").val();
        var czr = $czrDiv.find("input[name = 'bidderFileInfo.czrId']").val();
        if (!isNull(czr) && !isNull(sgef) && !isNull(gef)) {
            return true;
        } else {
            if (isNull(czr)) {
            ...
            if (isNull(sgef)) {
                $sgefDiv.find(".fileInput").css("border", "1px solid rgb(230, 17, 17)");
            }
            if (isNull(gef)) {
                $gefDiv.find(".fileInput").css("border", "1px solid rgb(230, 17, 17)");
            }
                        ...
                        if (data.code === "1") {
                            layer.msg("文件上传成功", {
                                icon: 1, 'time': 2000, 'end': function () {
                                    goToUrl('${ctx}/bidFile/bidFileUpLoadPage', $(".bidFileLi"));
                                }
                            });
