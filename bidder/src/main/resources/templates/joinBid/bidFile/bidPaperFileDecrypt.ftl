...
<div class="right-center" style="display: none">
    ...
    <div class="decryptContent">
        <#if bidderFileInfo.gefId??>
            <div class="space yckFile">
                <label for="">投标文件（yck）</label>
                <@UploadOneTag name="gefId" allowFileSize="1024M" allowType="*.yck;"
                autocomplete="off" class="fileInput layui-input" style="float:left;" disabled="disabled">
                </@UploadOneTag>
                <div onclick="decryptYck(${bidderFileInfo.gefId!})" class="decryptBtn">解密</div>
            </div>
        </#if>
        <div class="space syckFile">
            <label for="">投标文件（syck）</label>
            <@UploadOneTag name="sgefFileId" allowFileSize="1024M" allowType="*.syck;"
            autocomplete="off" class="fileInput layui-input" style="float:left;" disabled="disabled">
            </@UploadOneTag>
            <div onclick="decryptSyck(${bidderFileInfo.sgefId!})" class="decryptBtn">解密</div>
        </div>
    </div>
</div>
...
<script>
    ...
    function decrypt(fileId, type, privateKey, isOtherCa) {
        doLoading();
        $.ajax({
            url: "${ctx}/bidderModel/paperDecrypt",
            type: "POST",
            cache: false,
            data: {
                fileId: fileId,
    ...
    //判断是否已经上传文件,是则显示数据
    function isUpload() {
        if (!isNull('${yckFile}') && '${yckFile.id}' != null) {
            $(".yckFile").find(".fileInput").val('${yckFile.name}');
            $(".yckFile").find("input[name = 'gefFileId']").val('${yckFile.id}');
        }
        if (!isNull('${syckFile}') && '${syckFile.id}' != null) {
            $(".syckFile").find(".fileInput").val('${syckFile.name}');
            $(".syckFile").find("input[name = 'sgefFileId']").val('${syckFile.id}');
        }
    }
</script>