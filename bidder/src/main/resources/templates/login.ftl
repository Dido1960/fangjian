...
<div class="box">
    ...
    <div class="center">
        <div class="left">
            <h3>投标企业</h3>
            <img src="${ctx}/img/bidder.png" alt="">
        </div>
        <div class="right">
            <div class="tlt">
            ...
            <div class="puForm">
                <form id="userPassForm" class="layui-form" action="${ctx}/login" method="post">
                    ...
                    <div class="useinput">
                        ...
                        <div class="visitor" onclick="visitorLogin()">游客登录</div>
                    </div>
                </form>
            </div>
                ...
                <form id="caForm" class="layui-form" action="${ctx}/login" method="post">
                    ...
                    <div class="caLigon">
                        <div style="height: 20%;width: 85%">
                            <select id="ca_list" name="casn" placeholder="请选择登录CA" class="list" style="height: 20%;"
                                    lay-filter="casnSelect" readonly="readonly">
                            </select>
                        </div>
                        <div class="refresh">
...
<#--游客登录 -->
<div class="puForm" id="visitorLogin" style="display: none">
    <form id="visitor" class="layui-form" method="post">
        <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
        <input type="hidden" name="visitorLogin" value="1">
        <div class="useinput">
            <div class="photo">
                <input type="text" id="visitorCode" class="verify" name="vrifyCode" autocomplete="false"
                       lay-verType="tips"
                       lay-verify="required"
                       lay-reqText="请输入验证码" placeholder="验证码" maxlength="4">
                <img onclick="initVerification()" class="yzm" style="cursor: pointer;">
            </div>
            <div class="login" lay-submit lay-filter="visitorLogin*">登&nbsp;&nbsp;&nbsp;&nbsp;录</div>
        </div>
    </form>
</div>
<!-- 处理登录错误信息 -->
...
<script>
    ...
    $(window).on("load", function () {
        ...
        layui.use('form', function () {
            ...
            // 游客登录
            form.on('submit(visitorLogin*)', function (data) {
                console.log(serializeObject($('#visitor')));
                layer.msg("数据加载中...", {icon: 16, time: 0, shade: 0.2});
                setTimeout(function () {
                    $.ajax({
                        url: '${ctx}/login',
                        type: 'post',
                        cache: false,
                        data: serializeObject($('#visitor')),
                        success: function (data) {
                ...
                if (!isNull(ukey_serial_number)) {
                    caLogin(ukey_serial_number, $password.val(), function (returnInfo) {
                        getQSKJCertInfo(ukey_serial_number, function (QSKJ_INFO) {
                            ...
                            if(QSKJ_INFO.QSKJ_COMPANY_KEY != "1"){
                                layerAlert("非投标锁！无登录权限")
                                return;
                            }
                            var index = layer.msg("数据加载中...", {icon: 16, time: 0, shade: 0.2});
                            $.ajax({
