...
<div class="cont-left">
    ...
    <div class="score">
        ...
        <div id="grade-content" class="bottom layui-form">
            <ol>
                        ...
                        <li>
                            <div class="long lis">
                                ...
                                <div class="inps" id="item_${item.id}">
                                    <div class="radio">
                                        <div class="review pass <#if expertReview.enabled == 1>disabled</#if>"
                                             data-result="1">
                                            <div class="circular">
                                                <i></i>
                                            </div>
                                        ...
                                        <div class="review unpass  <#if expertReview.enabled == 1>disabled</#if>"
                                             data-result="0">
                                            <div class="circular">
                                                <i></i>
                                            </div>
        ...
        <form id="review-form" action="${ctx}/expert/preBidEval/personalReviewEnd" method="post">
...
<script >
    ...
    function getBidderData(bidderId) {
        ...
        $('.reasonText').slideUp();
        $.ajax({
            url: "${ctx}/expert/evalPlan/getBidderData",
            data: {
    ...
    function echoScore(data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (score.evalResult == 0) {
                $unpass.addClass("chioce");
                $textArea.slideDown();
            } else if (score.evalResult == 1) {
                $pass.addClass("chose");
                $grade.find(".pass").addClass("chioce");
            } else {
                $textArea.val('');
            }
        }
    ...
    function showExpertEval() {
        parent.window.location.href = "${ctx}/expert/preBidEval/evalResultBasePage";
    }
...
<script>
        ...
        $.ajax({
            ...
            success: function () {
                $(".radio").each(function () {
                    ...
                    pass.addClass("chioce").siblings().removeClass("chioce");
    ...
    function personalReviewEnd() {
        $.ajax({
            url: "${ctx}/expert/preBidEval/validQualifiedEnd",
            type: "POST",
            cache: false,
            success: function (result) {
                if (result.code == "1") {
     ...
     * 校验是否有不合格的评分项
     */
    function validUnqualified() {
        layerConfirm("确认要结束个人评审吗？结束后将无法修改评审结果！", function () {
            $.ajax({
                url: "${ctx}/expert/preBidEval/validUnqualified",
                type: "POST",
                cache: false,
                success: function (result) {
                    if (!result) {
                        // 有不合格项，需填写意见才能结束
                        setTimeout(jumpOpinionPage, 200);
                    } else {
                        // 没有不合格项，直接结束
                        personalEnd();
                    }
    ...
    function personalEnd() {
        $.ajax({
            url: "${ctx}/expert/preBidEval/personalReviewEnd",
            type: "POST",
            cache: false,
            success: function (result) {
                if (result) {
     ...
     * 跳转意见填写页面
     */
    function jumpOpinionPage() {
        hide_IWeb2018();
        window.top.layer.open({
            type: 2,
            title: "填写意见",
            content: "${ctx}/expert/preBidEval/jumpOpinionPage?bidSectionId=${bidSection.id}&readOnly=false",
            area: ['70%', '70%'],
     ...
     * 评审重评
     * */
    function callPersonReview() {
        layerConfirm("确认要重评该环节吗? 确认后请通知其他评标专家刷新本页面!", function () {
            $.ajax({
                url: "${ctx}/expert/preBidEval/callPersonReview",
                type: "POST",
                cache: false,
                success: function () {
                    pageReloade();
    ...
    function endGroupReview() {
        $.ajax({
            url: "${ctx}/expert/preBidEval/validGroupReview",
            type: "POST",
            cache: false,
            success: function (data) {
                if (data.code === "2") {
    ...
    function submitResult(msg) {
        layerConfirm(msg, function (index) {
            ...
            $.ajax({
                url: "${ctx}/expert/preBidEval/endGroupReview",
                type: "POST",
                success: function (data) {
                    setTimeout(function () {
                        loadComplete();
