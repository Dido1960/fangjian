                    ...
                    <#list bidder.expertReviewSingleItems as expertReviewSingleItem>
                        <#if expertReviewSingleItem ??>
                            <tr>
                                ...
                                <td>
                                    <textarea onblur="checkEvalComments(this)" maxlength="200" data-evalid="${expertReviewSingleItem.id}" >${expertReviewSingleItem.evalComments}</textarea>
                                </td>
                            </tr>
                        </#if>
                    </#list>
...
<script type="text/javascript">
    ...
    function checkOpinion(obj){
        ...
        if (opinion.length >= 255){
            $(obj).css('background','#FFE7E7');
            layerAlert("输入理由过长，系统自动保存前255个字");
            // 防止理由过长，导致异常
            $(obj).val(opinion.substr(0,255))
        }
