...
<section>
    <div class="head">
        <h3><span onclick="window.location.href='${ctx}/expert/epcBidEval/qualifyReviewPage'">返回</span>评审结果</h3>
    </div>
    <div class="choice">
        <ol class="select">
...
<script>
    ...
    function loadPersonDetailed() {
        $("#grade_base").load("${ctx}/expert/epcBidEval/showPersonReviewResultPage",
            function () {
                // 添加跨域参数
                $.ajaxSetup({
                    type: "POST",
    ...
    function loadGroupDetailed() {
        $("#grade_base").load("${ctx}/expert/epcBidEval/showGroupReviewResultPage",
            function () {
                // 添加跨域参数
                $.ajaxSetup({
                    type: "POST",
