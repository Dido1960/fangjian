...
<script>
    ...
    function getResultData() {
        doLoading();
        $("#dataShowInfo tbody td").html("-")
        $.ajax({
            url: "${ctx}/expert/conBidEval/getResultData",
            data: {
    ...
    function echoScore(data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (score.evalResult == 0) {
                $grade.append("<span class='red-f red-s'>扣分</span>");
            } else if (score.evalResult == 1) {
                $grade.append("<span class='green-f green-s'>不扣分</span>");
            }
        }
    }
</script>
<script>
    ...
    function updatePersonResult(e) {
        ...
        var open_form = layer.open({
            ...
            content: "${ctx}/expert/conBidEval/updateDeductPersonResultPage?deductId=" + deductId,
            area: ['650px', '510px'],
        });
    }
