...
<head>
    ...
    <link rel="stylesheet" href="${ctx}/css/conDetailedResult.css">
</head>
<body>
...
<script>
    ...
    function loadPersonDetailed() {
        doLoading();
        $("#grade_base").load("${ctx}/expert/conBidEval/detailedPersonPage",
            function () {
                // 添加跨域参数
                $.ajaxSetup({
                    type: "POST",
    ...
    function loadGroupDetailed() {
        $("#grade_base").load("${ctx}/expert/conBidEval/detailedGroupResultBasePage",
            function () {
                // 添加跨域参数
                $.ajaxSetup({
                    type: "POST",
