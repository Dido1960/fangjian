        ...
        <ul id="check-number-li">
            <#list grades as grade>
                <li <#if grade_index == 0>class="sele"</#if> >
                    <a href="#">
                        <div class="flow-left" <#if grade_index == 0>style="opacity: 0;"</#if> ></div>
                        <div class="flow-num" onclick="loadGradeDetailed('${grade.id }')">${grade_index + 1}
                            <#if grade_index gt 1>
                                <i></i>
                            </#if>
                        </div>
                        <#if grades?size != (grade_index + 1)>
...
<script>
    ...
    function loadGradeDetailed(gradeId) {
        ...
        setTimeout(function () {
            ...
            iframe.src = "${ctx}/expert/conBidEval/loadDetailedGradeDetailed?gradeId=" + gradeId;
