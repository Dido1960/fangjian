...
<body>
<form id='opinionForm' style="margin-bottom: 90px">
    <table class="layui-table">
        ...
        <#if bidders ??>
            <#list bidders as bidder>
                <#if bidder.expertReviewSingleItemDeducts ??>
                    <#list bidder.expertReviewSingleItemDeducts as deduct>
                        <#if deduct ??>
                            <tr>
                                <#if deduct_index == 0>
                                    <#assign num = num + 1>
                                    <td rowspan="${bidder.expertReviewSingleItemDeducts?size }">${num }</td>
                                    <td rowspan="${bidder.expertReviewSingleItemDeducts?size }">${bidder.bidderName }</td>
                                </#if>
                                <td style="text-align: left">${deduct.gradeItem.itemContent}</td>
                                <td>
                                    <textarea onblur="checkOpinion(this)"  data-id="${deduct.id}" maxlength="200" >${deduct.deductComments}</textarea>
                                </td>
                            </tr>
                        </#if>
                    </#list>
...
<script type="text/javascript">
    ...
    function saveOpinion() {
        ...
        if (num>0) {
            ...
            $.ajax({
                url: "${ctx}/expert/conBidEval/updateDeducts",
