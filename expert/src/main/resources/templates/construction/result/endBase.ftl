...
<section>
    <div class="process">
        <h3><span onclick="returnUpFlow()" >返回</span><p>${bidSection.bidSectionName}</p></h3>
        <ul>
            <li id="rankingSummary" onclick="goToUrl('${ctx}/expert/conBidEval/rankingSummaryPage',this,null)">
...
<script>
    ...
    function checkSelect(obj) {
        $.ajax({
            ...
            success: function (data) {
                if (data.code == 2) {
                    goPdfToUrl('${ctx}/expert/conBidEval/evaluationReportPage',obj,"正在生成评标报告...");
                }else {
                    goToUrl('${ctx}/expert/conBidEval/evaluationReportPage',obj,null);
                }
            },
            error:function (e) {
                console.error(e);
    ...
    function checkReport(obj) {
        $.ajax({
            ...
            success: function (data) {
                if (data.code === "2") {
                    layerLoading("尚未生成评标报告，无法结束评标",2,2,null,null,null);
                }else if (data.code === "1") {
                    goToUrl('${ctx}/expert/conBidEval/evaluationEndPage',obj,null)
                }
            },
            error:function (e) {
                console.error(e);
