...
<head>
    ...
    <link rel="stylesheet" href="${ctx}/css/evalResultMutual.css">
</head>
<body>
<#include "${ctx}/include/header.ftl">
<section>
    <div class="head">
        <h3><span onclick="window.location.href='${ctx}/expert/conBidEval/otherMutualReviewIndex'">返回</span>评审结果</h3>
    </div>
    <div class="choice">
        <ol class="select">
...
<script>
    ...
    function loadPersonDetailed() {
        doLoading();
        $("#grade_base").load("${ctx}/expert/conBidEval/showPersonMutualResultPage",
            function () {
                // 添加跨域参数
                $.ajaxSetup({
                    type: "POST",
    ...
    function loadGroupDetailed() {
        $("#grade_base").load("${ctx}/expert/conBidEval/showGroupMutualResultPage",
            function () {
                // 添加跨域参数
                $.ajaxSetup({
                    type: "POST",
