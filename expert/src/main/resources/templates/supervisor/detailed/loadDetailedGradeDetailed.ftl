                ...
                <table cellpadding=0 cellpadding=0>
                    ...
                    <tbody>
                        <#list grade.gradeItems as item>
                            <tr>
                                <td class="long" style="text-align: left">&emsp;&emsp;${item_index + 1}、${item.itemContent}(总分：${item.score}分)
                                    <#if item.scoreType == 'fixed'>
                                        <p>（取值范围：${item.scoreRange?replace(",", ",&nbsp;&nbsp;")}）</p>
                                    <#else >
                                        <p>（请酌情打分）</p>
                                    </#if>
                                </td>
...
<script>
    ...
    function getBidderData(bidderId) {
        $.ajax({
            url: "${ctx}/expert/supBidEval/getBidderData",
            data: {
                "bidderId": bidderId,
    ...
    function showExpertEval() {
        parent.window.location.href = "${ctx}/expert/supBidEval/evalResultDetailedIndex";
...
<script>
    ...
    function validReviewComplete() {
        ...
        $.ajax({
            url: "${ctx}/expert/supBidEval/validBidderScoreCompletion",
            data: {
                "bidderId": bidderId,
                "reviewType": '${reviewType}'
            },
    ...
    function oneKeyNoDeduct() {
        layerLoading();
        $.ajax({
            url: "${ctx}/expert/supBidEval/oneKeyNoDeduct",
            type: "POST",
            cache: false,
            success: function () {
    ...
    function personalReviewEnd() {
        $.ajax({
            url: "${ctx}/expert/supBidEval/checkSupPersonalEnd",
            type: "POST",
            cache: false,
            success: function (result) {
                if (result.code == "1") {
    ...
    function supPersonalEnd() {
        layerConfirm("确认要结束个人评审吗？结束后将无法修改评审结果！", function () {
            $.ajax({
                url: "${ctx}/expert/supBidEval/supPersonalEnd",
                type: "POST",
                cache: false,
                success: function (result) {
                    if (result) {
    ...
    function restartDetailedReview() {
        layerConfirm("确认要重评该环节吗? 确认后请通知其他评标专家刷新本页面!", function () {
            $.ajax({
                url: "${ctx}/expert/supBidEval/restartDetailedReview",
                type: "POST",
                cache: false,
                success: function () {
                    pageReloade();
    ...
    function validDetailedGroupEnd() {
        $.ajax({
            url: "${ctx}/expert/supBidEval/checkSupGroupEnd",
            type: "POST",
            cache: false,
            success: function (data) {
                if (data.code === "2") {
    ...
    function submitResult(msg) {
        layerConfirm(msg, function (index) {
            ...
            $.ajax({
                url: "${ctx}/expert/supBidEval/endSupGroupReview",
                type: "POST",
                success: function (data) {
                    setTimeout(function () {
                        loadComplete();
