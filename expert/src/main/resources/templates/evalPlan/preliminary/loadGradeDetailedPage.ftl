                        ...
                        <li>
                            <div class="long lis">
                                ...
                                <div class="inps" id="item_${item.id}">
                                    ...
                                    <textarea class="reasonText" style="display: none"
                                              <#if expertReview.enabled == 1>disabled</#if> onblur="saveComment(this)" maxlength="200"></textarea>
                                </div>
                            </div>
...
<div class="cont-right">
    <ol class="tit pdf-file-list" id="file-type-li">
        ...
        <li class="choice" data-pdf-index="3" fdfsMark="/bidderFile/{bidDocId}/resources/TempConvert/BStemp.pdf">
            商务标文件
        </li>
...
<script>
    ...
    function evalResultPage() {
        parent.window.location.href = "${ctx}/expert/evalPlan/qualifiedEvalResultPage";
    }
...
<script>
    ...
    function personalReviewEnd() {
        $.ajax({
            url: "${ctx}/expert/evalPlan/validQualifiedEnd",
            type: "POST",
            cache: false,
            success: function (result) {
                if (result.code == "1") {
    ...
    function checkUnqualified() {
        layerConfirm("确认要结束个人评审吗？结束后将无法修改评审结果！", function () {
            $.ajax({
                url: "${ctx}/expert/evalPlan/validUnqualified",
                type: "POST",
                cache: false,
                success: function (result) {
                    if (!result) {
    ...
    function personalEnd() {
        $.ajax({
            url: "${ctx}/expert/evalPlan/personalReviewEnd",
            type: "POST",
            cache: false,
            success: function (result) {
                if (result) {
    ...
    function opinionPage() {
        ...
        window.top.layer.open({
            type: 2,
            title: "填写意见",
            content: "${ctx}/expert/evalPlan/jumpOpinionPage",
            area: ['70%', '70%'],
    ...
    function restartReview() {
        layerConfirm("确认要重评该环节吗? 确认后请通知其他评标专家刷新本页面!", function () {
            $.ajax({
                url: "${ctx}/expert/evalPlan/callPersonReview",
                type: "POST",
                cache: false,
                success: function () {
                    pageReloade();
    ...
    function validDetailedGroupEnd() {
        $.ajax({
            url: "${ctx}/expert/evalPlan/validGroupReview",
            type: "POST",
            cache: false,
            success: function (data) {
                if (data.code === "2") {
    ...
    function submitResult(msg) {
        layerConfirm(msg, function (index) {
            ...
            $.ajax({
                url: "${ctx}/expert/evalPlan/endGroupReview",
                type: "POST",
                success: function (data) {
                    setTimeout(function () {
                        loadComplete();
