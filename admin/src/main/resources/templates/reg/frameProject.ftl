...
<div id="LAY_app">
    <div class="layui-col-md6 layui-layout layui-layout-admin">
        <div class="layui-card" style="margin: 15px 0 0 15px">
            <p style="text-align: center;background-color: #f2f2f2;font-size: 18px;padding-bottom: 10px">项目查看权限配置</p>
...
<script type="text/javascript">
    ...
    function menuPermission(data) {
        ...
        layui.use('table', function () {
            ...
            tableIns1 = table.render({
                ...
                url: '${ctx}/wordbook/listWordBook?userId='+data.id+'&type=1',
                // page: { //支持传入 laypage 组件的所有参数（某些参数除外，如：jump/elem） - 详见文档
                //     layout: ['limit', 'count', 'prev', 'page', 'next', 'skip'] //自定义分页布局
                //     // ,curr: 5 //设定初始在第 5 页
                //     ,groups: 3 //只显示 1 个连续页码
    ...
    function updateUserReg() {
        layer.confirm("您确定要修改吗？", {
            ...
            console.log(arr,"-----------------")
            $.ajax({
                url: '/admin/govUserExamine/updateAuditSiteKeyByUserId',
                type: 'post',
                cache: false,
                async: true,
