package com.ejiaoyi.supervise.auth;
...
public class CustomUserDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String param) throws AuthenticationException {
        ...
        if (!CommonUtil.isEmpty(loginName))  {
            user = govUserService.getGovUserByLoginName(loginName, GovDepType.TENDER_OFFICE.getType());
            if (user == null) {
        ...
        if (!CommonUtil.isEmpty(ukey_serial_number)) {
            user = govUserService.getGovUserByCasn(ukey_serial_number, GovDepType.TENDER_OFFICE.getType());
            if (user == null) {
