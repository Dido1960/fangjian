...
<head>
    ...
    <link rel="stylesheet" href="${ctx}/css/information1.css">
...
<section>
    <div class="left">
        <div class="live">
            ...
            <div class="live-foot">
                <span class="name">代理机构：${bidSection.tenderAgencyName}</span><span
                        class="phone">联系电话：${bidSection.tenderAgencyPhone}</span>
            </div>
        </div>
...
<script>
    ...
    function checkOpenEnd() {
        $.ajax({
            url: '${ctx}/bidOpenHall/checkOpenEnd',
            type: 'post',
            cache: false,
            success: function (data) {
                if (data) {
                    layer.msg('开标结束', {icon: 1}, function () {
                        window.location.href = '/gov/bidOpen/bidProgress'
                    })
                }
            }, error: function (data) {
                console.warn(data);
